...
「インターネット非公開を前提に、書き込み token を廃止して認証境界をネットワーク到達制御 + proxy に寄せる」方針で対応した（コード実装 + docs 更新を同一 PR で実施）。セキュリティと運用のバランス検討の結論:
...
4. **バイナリ側に `ReadTimeout` / `WriteTimeout` / `IdleTimeout` を追加** — slow-body / slow-loris を安価に切る（本格的なレート制限は proxy 責務）。
...
実装範囲: `cmd/agent-telemetry-server/main.go`（token 撤去・bind 既定・公開警告・timeout）、`internal/serverpipe/handler.go`（`checkAuth` / `Token` フィールド撤去）、`internal/serverpipe/handler_test.go`、`docs/spec.md` / `docs/design.md` / `site/content/setup/server/`。
