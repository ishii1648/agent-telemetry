...
- **将来オプションの列挙**（per-user isolation を約束するデプロイ向け、実装可否は別 PR）: per-user token / token scoping / 監査ログ / backend カーディナリティ制御。
- **isolation を約束するデプロイ向け要件の切り分け**: テナント間・ユーザ間分離を SLA として謳う運用は、上記将来オプションのいずれかを前提条件とすると注記。
