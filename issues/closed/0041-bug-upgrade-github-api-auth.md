...
`internal/upgrade` の GitHub Releases latest API request 作成を helper 化し、`GITHUB_TOKEN` または `gh auth token` で取得した token がある場合に `Authorization: Bearer` header を付与するようにした。token が取れない環境では未認証 request へフォールバックする。

単体テストでは header 付与、空 token 時の未認証 request、`GITHUB_TOKEN` 優先、`gh auth token` fallback、token 取得失敗時の未認証 fallback を固定した。
