...
2. **Tier 3 parity**: session-grain export（[0053] の Tier 3）が無く、otel dashboard は SQLite dashboard と feature parity に達していない（[0053] open）。
...
- 規模が大きいので、実装着手時に runtime cutover（1）と Tier 3（2, 既存 [0053]）を独立 PR / 子 issue に切ることを検討する。本 issue はローカル経路移行の傘として全体像と順序を保持する。
