...
> **direct は OTLP SDK / protobuf exporter への切替が必要**（現行の手組み JSON poster + 固定 Bearer では Datadog OTLP intake を満たせない）。この client 側拡張は [0042] の範囲。本ディレクトリは Datadog 側の受け設定のみを扱う。
