-- Derived relations over the events table (see schema.sql). This file is fully
-- idempotent and NON-DESTRUCTIVE to stored data: it only DROPs and recreates
-- VIEWs and the INSTEAD OF INSERT triggers, never the events table. That makes
-- it safe to reapply on a populated DB to repair a missing aggregate VIEW
-- (schema.EnsureViews), unlike the full schema.SQL which drops events.
